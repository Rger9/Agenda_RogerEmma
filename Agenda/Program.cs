...
namespace Agenda
{
    class Program
    {
        ...
        static void RecuperarUsuari()
        {
            ...
            string[] linies = File.ReadAllLines("agenda.txt");

            bool usuariTrobat = false;

            for (int i = 0; i < linies.Length; i++)
            {
                if (linies[i].StartsWith(nomABuscar, StringComparison.OrdinalIgnoreCase))
                {
                    Console.WriteLine($"Usuari trobat: {linies[i]}");
                    usuariTrobat = true;
                    System.Threading.Thread.Sleep(5000); // Espera 5 segons
                    Agenda();
                }
            }
            ...
            if (!usuariTrobat)
            {
                Console.WriteLine("L'usuari introdüit no a estat trobat.");

                Console.Write("Vols buscar un altre usuari? (si/no): ");
                string resposta = Console.ReadLine();

                if (resposta.Equals("si", StringComparison.OrdinalIgnoreCase))
                {
                    RecuperarUsuari();
                }
            }
        ...
        static void ModificarUsuari()
        {
            ...
            string nomAModificar = Console.ReadLine();
        }
        ...
        static void EliminarUsuari()
        {
            ...
            string nomAEliminar = Console.ReadLine();

            string rutaFitxer = "agenda.txt";

            List<string> linies = new List<string>(File.ReadAllLines(rutaFitxer));
            int indexUsuari = -1;

            for (int i = 0; i < linies.Count; i++)
            {
                if (linies[i].StartsWith(nomAEliminar, StringComparison.OrdinalIgnoreCase))
                {
                    indexUsuari = i;
                    Agenda();
                }
            ...
            linies.RemoveAt(indexUsuari);

            // Sobreescriu el fitxer amb la llista actualitzada
            File.WriteAllLines(rutaFitxer, linies);

            Console.WriteLine($"Usuari eliminat: {linies[indexUsuari]}");
        }
        ...
        {
            Console.WriteLine("Aquest mètode ens mostrarà els usuaris de l’agenda i els telèfons ordenats de forma alfabètica.");
        }
